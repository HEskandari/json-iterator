...
func Test_read_object_as_any(t *testing.T) {
	...
	any := jsoniter.DefaultAPI().Get([]byte(`{"a":"stream","c":"d"}`))
...
func Test_object_lazy_any_get_all(t *testing.T) {
	should := require.New(t)
	any := jsoniter.DefaultAPI().Get([]byte(`{"a":[0],"stream":[1]}`))
...
func Test_any_within_struct(t *testing.T) {
	...
	obj := TestObject{}
	err := jsoniter.DefaultAPI().UnmarshalFromString(`{"Field1": "hello", "Field2": [1,2,3]}`, &obj)
	should.Nil(err)
